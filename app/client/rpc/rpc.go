...
func (r *rpcClient) call(ctx context.Context, addr string, req client.Request, resp interface{}, opts client.CallOptions) error {
	...
	if err != nil {
		return errors.InternalServerError("nitro", "connection error: %v", err)
	}
	...
	go func() {
		defer func() {
			if r := recover(); r != nil {
				ch <- errors.InternalServerError("nitro", "panic recovered: %v", r)
			}
		}()
	...
	case err := <-ch:
		return err
	case <-ctx.Done():
		grr = errors.Timeout("nitro", fmt.Sprintf("%v", ctx.Err()))
	}
...
func (r *rpcClient) stream(ctx context.Context, addr string, req client.Request, opts client.CallOptions) (client.Stream, error) {
	...
	if err != nil {
		return nil, errors.InternalServerError("nitro", "connection error: %v", err)
	}
...
func (r *rpcClient) Call(ctx context.Context, request client.Request, response interface{}, opts ...client.CallOption) error {
	...
	select {
	case <-ctx.Done():
		return errors.Timeout("nitro", fmt.Sprintf("%v", ctx.Err()))
	default:
	}
	...
	// return errors.New("nitro", "request timeout", 408)
	call := func(i int) error {
		// call backoff first. Someone may want an initial start delay
		t, err := callOpts.Backoff(ctx, request, i)
		if err != nil {
			return errors.InternalServerError("nitro", "backoff error: %v", err.Error())
		}
	...
	for i := 0; i <= retries; i++ {
		...
		select {
		case <-ctx.Done():
			return errors.Timeout("nitro", fmt.Sprintf("call timeout: %v", ctx.Err()))
		case err := <-ch:
...
func (r *rpcClient) Stream(ctx context.Context, request client.Request, opts ...client.CallOption) (client.Stream, error) {
	...
	select {
	case <-ctx.Done():
		return nil, errors.Timeout("nitro", fmt.Sprintf("%v", ctx.Err()))
	default:
	}
	...
	call := func(i int) (client.Stream, error) {
		...
		t, err := callOpts.Backoff(ctx, request, i)
		if err != nil {
			return nil, errors.InternalServerError("nitro", "backoff error: %v", err.Error())
		}
	...
	for i := 0; i <= retries; i++ {
		...
		select {
		case <-ctx.Done():
			return nil, errors.Timeout("nitro", fmt.Sprintf("call timeout: %v", ctx.Err()))
		case rsp := <-ch:
			// if the call succeeded lets bail early
			if rsp.err == nil {
